...
class MainWindow(QMainWindow):
    ...
    def choose_start_position(self, button):
        ...
        if figure_type == self.type_of_figure: 
            ...
            if self.parent_r != -1 and self.parent_c != -1:
                ...
                parent_button.setStyleSheet(style2)
            ...
            result = []
            self.get_buttons_that_should_cut_others(list_buttons_that_must_to_cut_ohter,self.board[self.parent_r][self.parent_c].type_of_figure)
            
            self.get_all_ways(self.parent_r, self.parent_c, ways, more_important_ways, check_is_qeen, self.board[self.parent_r][self.parent_c].type_of_figure)
            print()
            print("more_importants_way")
            for t in more_important_ways:
                print(t)
            if check_is_qeen:
                if self.pos_which_can_cut_figure(more_important_ways, result, self.board[self.parent_r][self.parent_c].type_of_figure):
            ...
            if len(list_buttons_that_must_to_cut_ohter) == 0:
                if len(more_important_ways) == 0 and len(ways) > 0:
                    self.change_position(ways, figure_type, index_of_row, index_of_col,self.board[self.parent_r][self.parent_c].type_of_figure, is_capture=False,)
            else:
                # Если есть обязательные взятия
                if figure_type == None and [self.parent_r, self.parent_c] in list_buttons_that_must_to_cut_ohter:
                    if [index_of_row, index_of_col] in more_important_ways:
                        ...
                        #вычисление напрваления сбиваемой шашки
                        ...
                        while (time_r, time_c)!= (index_of_row, index_of_col) and  0 <= time_r < 8 and 0 <= time_c < 8:
                            ...
                            time_r+=d_row
                            time_c+=d_col
                        # cut_row = (index_of_row + self.parent_r) // 2
                        # cut_col = (index_of_col + self.parent_c) // 2
                        name_of_cut = f"button_{cut_row}{cut_col}"
                        cut_button = self.get_button_by_name(name_of_cut)
                        
                        # Очищаем срубленную фигуру
                        ...
                        self.change_position(more_important_ways, figure_type, index_of_row, index_of_col,self.board[self.parent_r][self.parent_c].type_of_figure, is_capture=True)
    ...
    def change_position(self, ways, figure_type, index_of_row, index_of_col, types,is_capture):
        if figure_type == None and [index_of_row, index_of_col] in ways:
            ...
            if is_capture:
                ...
                self.get_all_ways(index_of_row, index_of_col, ways, more_important_ways, self.board[index_of_row][index_of_col].isqeen,current_type)
                if len(more_important_ways) > 0:
                    self.name_parent_button = name
                    self.parent_r = index_of_row
                    self.parent_c = index_of_col
                    new_button.setStyleSheet(style1)
                    return
    ...
    def get_all_ways(self, row, col, ways, more_important_ways, is_qeen, types):
        ...
        if not is_qeen:
            ...
            count=1
            while (row-count>-1 and col-count>-1) or (row-count>-1 and col+count<8) or (row+count<8 and col-count>-1) or (row+count<8 and col+count<8):
                if row-count>-1 and col-count>-1 and self.board[row-count][col-count].type_of_figure is None:
                    ways.append([row-count, col-count])
                if row-count>-1 and col+count<8 and self.board[row-count][col+count].type_of_figure is None:
                    ways.append([row-count, col+count])
                if row+count<8 and col-count>-1 and self.board[row+count][col-count].type_of_figure is None:
                    ways.append([row+count, col-count])
                if row+count<8 and col+count<8 and self.board[row+count][col+count].type_of_figure is None:
                    ways.append([row+count, col+count])
                count+=1
    ...
    def pos_which_can_cut_figure(self, more_important_ways, result,types):
        for r, c in more_important_ways:
            ...
            if len(new_more_important_ways)> 0 and r!=self.parent_r and c!=self.parent_c:
                print()
                print("куда дальше становиться")
                for s in new_more_important_ways:
                    print(s)
                result.append([r, c])
        return len(result) > 0
